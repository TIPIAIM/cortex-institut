...
const FooterContainer = styled.footer`
  position: relative;
  background:
    radial-gradient(900px 300px at 85% -10%, ${colors.semygsecondar}55, transparent 60%),
    linear-gradient(180deg, #0e1a2b, #0f223a);
...
const LinkList = styled.div`
  ...
  a:hover { border-bottom-color: #27406688; color: ${colors.accentGold}; }
`;
...
const Input = styled.input`
  ...
  outline: none;
  &::placeholder{ color:#93a4bb; }
  &:focus{ border-color:${colors.semygsecondar}; box-shadow: 0 0 0 3px rgba(42,75,124,.25); }
`;
const Button = styled(motion.button)`
  appearance:none; cursor:pointer;
  border-radius: 12px; border:1px solid #d9b642;
  background: ${colors.accentGold}; color:#0e1a2b; padding: 12px 16px; font-weight: 800;
  box-shadow: 0 8px 22px rgba(242,201,76,.24);
  transition: box-shadow .15s ease;
  &:hover{ box-shadow: 0 10px 26px rgba(242,201,76,.30); }
`;
...
const LegalLink = styled(motion.button)`
  ...
  color: ${colors.accentGold}; display:inline-flex; align-items:center; gap:8px;
  font-size: .92rem;
  &:hover{ color:${colors.text}; }
`;

const Copy = styled(motion.div)`
  text-align:center; color:${colors.muted}; font-size:.9rem;
`;
...
const ModalContent = styled(motion.div)`
  ...
  background: #0b1729; color: ${colors.text};
  border: 1px solid #274066; border-radius: 16px;
  box-shadow: 0 10px 32px rgba(0,0,0,.35);
  padding: clamp(16px, 2.4vw, 22px);
`;
const Close = styled.button`
  position:absolute; top:10px; right:10px;
  appearance:none; background:transparent; border:1px solid #274066;
  color:${colors.text}; border-radius:10px; padding:6px; cursor:pointer;
  &:hover{ border-color:${colors.accentGold}; color:${colors.accentGold}; }
`;
const ModalTitle = styled.h5`
  margin: 0 0 8px; font-size: 1.05rem; color:${colors.accentGold};
`;
const ModalBody = styled.div`
  font-size:.95rem; line-height:1.6; color:${colors.accentGoldLight};
  h6{ margin:10px 0 6px; color:${colors.text}; font-size:.95rem; }
    ...
    mentions: {
      ...
      body: (
        <>
          <h6>Éditeur</h6>
          <p>
            Institut CORTEX (Groupe NONI)<br/>
            Adresse : Kobaya, Conakry (GN)<br/>
            Contact : direction@institut-cortex.com
          </p>
          <h6>Hébergement</h6>
          <p>Infrastructure cloud sécurisée (UE). Disponibilité & sauvegardes gérées.</p>
          <h6>Propriété intellectuelle</h6>
          <p>Tous contenus (textes, logos, images) restent la propriété d’Institut CORTEX.</p>
        </>
      ),
    },
    privacy: {
      ...
      body: (
        <>
          ...
          <h6>Vos droits</h6>
          <p>Accès, rectification, suppression : direction@institut-cortex.com</p>
        </>
      ),
    },
    faq: {
      ...
      body: (
        <>
          ...
          <h6>Admissions</h6>
          <p>Dossier, entretien éventuel et prérequis propres à chaque programme.</p>
          <h6>Certifications</h6>
          <p>Préparation à des référentiels reconnus et délivrance d’attestations.</p>
        </>
      ),
    },
    cgu: {
      ...
      body: (
        <>
          <h6>Objet</h6>
          <p>Définir l’usage du site et la qualité de service visée par CORTEX.</p>
          <h6>Responsabilités</h6>
          <p>Utilisation loyale et conforme ; CORTEX s’efforce d’assurer l’exactitude des informations.</p>
        </>
      ),
    },
  };
    ...
    <FooterContainer>
      ...
      >
        ...
        <HeaderRow variants={variants}>
          ...
          <p>
            Des programmes orientés impact, une pédagogie active, et des partenariats académiques
            pour transformer durablement vos compétences et vos organisations.
          </p>
        </HeaderRow>
        ...
        <GridContainer>
          ...
          <Column variants={variants}>
            ...
            <Line>
              <Mail size={18} />
              <a href={`mailto:${CONTACT.mailCommercial}`}>{CONTACT.mailCommercial}</a>
            </Line>
            <Line>
              <Mail size={18} />
              <a href={`mailto:${CONTACT.mailDirection}`}>{CONTACT.mailDirection}</a>
            </Line>
          ...
          <Column variants={variants}>
            ...
            <LinkList>
              <a href="/programmes"><BookOpen size={14} style={{marginRight:6}}/> Programmes</a>
               <a href="/apropos"><Users size={14} style={{marginRight:6}}/> À propos</a>
              <a href="/contact"><Mail size={14} style={{marginRight:6}}/> Contact</a>
            </LinkList>
          </Column>
          ...
          <Column variants={variants}>
            ...
            <Line><GraduationCap size={18}/> Formations & Masters en ligne</Line>
            <Line><ShieldCheck size={18}/> Certifications & qualité</Line>
            <Line><BookOpen size={18}/> Recherche appliquée & cas réels</Line>
            <Line><Users size={18}/> Partenariats académiques</Line>
          </Column>
      ...
      {modal && (
        <ModalBackdrop
          ...
          onClick={close}
          aria-modal="true" role="dialog" aria-labelledby="legal-title"
        >
          <ModalContent
          ...
          >
            <Close aria-label="Fermer" onClick={close}><X size={18} /></Close>
